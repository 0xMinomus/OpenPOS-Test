...
- `lib/localdb.ts` — data layer OFFLINE (localStorage `op_offline_db`, satu blok JSON + hook `useLocalDB`): produk/kategori/transaksi/settings/seq, backup export/import JSON. Aplikasi desktop Windows via **Electron** (bukan PWA): entry terpisah `offline.html` + `src/offline-main.tsx` (HashRouter, base `./`, build ke `dist-offline/` via `vite.offline.config.ts`), shell di `web/electron/` (main.cjs load `dist-offline/offline.html`, preload expose `window.offline.close`). Build installer: `npm run electron:build` → `electron-builder --win nsis` → `web/release-out/`. Struk dipakai bersama via `lib/receipt.tsx` (cloud POS + offline POS). Halaman `/unduh` = tombol unduh installer Windows (link GitHub Releases). **Catatan mesin ini:** app StablyAI "Orca" memonitor folder Documents dan mengunci dir build — kalau build gagal EPERM, keluar dari Orca atau build ke luar Documents via `--config.directories.output=<temp>`.
