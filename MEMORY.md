...
- `lib/ui.tsx` — komponen internal (Button, Input, Modal, Pill, Td/Th, PageHead, Empty, StatusPill) + **`TrxItems`** (chip produk + badge qty untuk list transaksi).
...
- **Dashboard** — admin: KPI (Omzet/Transaksi/Produk/Stok menipis), bar chart "Penjualan 7 hari terakhir" (hijau kalem `--chart-omzet`, grid + sumbu Y `fmtShort`), donut metode bayar, produk terlaris, list "Transaksi Terbaru" (produk + qty via `TrxItems`). Kasir: sapaan, kartu "Kasir siap" + Buka POS, 3 KPI hari ini, "Transaksi Saya".
