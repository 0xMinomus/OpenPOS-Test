...
- Versi saat ini: **1.0.2** (`web/release-out/OpenPOS Setup 1.0.2.exe`, installer ±99 MB, terpasang ±320 MB).
...
- **Release terakhir**: v0.1.0 (app offline awal, minim fitur) → v0.2.0 (1:1 webapp) → v0.2.3 (catch-up Stok/Transaksi/Laporan/Pengaturan + struk thermal) → v1.0.1 (fix zona waktu dashboard offline) → v1.0.2 (Dashboard + Produk 1:1 dengan cloud: chart biru, tanpa cursor, paging 15).
